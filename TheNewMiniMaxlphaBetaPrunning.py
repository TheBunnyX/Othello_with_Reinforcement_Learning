...
def alpha_beta(env, depth, alpha, beta, maximizing_player):
    ...
    if maximizing_player:
        ...
        for move in valid_moves:
            ...
            _, eval = alpha_beta(next_state, depth - 1, alpha, beta, False)
            if eval > max_eval:
                max_eval = eval
                best_move = move
        ...
        for move in valid_moves:
            next_state = env.clone()
            next_state.step(move)
            _, eval = alpha_beta(next_state, depth - 1, alpha, beta, True)
            if eval < min_eval:
                min_eval = eval
                best_move = move
            beta = min(beta, eval)
...
def play_vs_minimax_alpha_beta(env, episodes, depth):
    player1_wins = 0
    player2_wins = 0
    draws = 0
    ...
    for episode in range(1, episodes + 1):
        ...
        while not done:
            ...
            if current_player == BLACK:
                action, _ = alpha_beta(env, depth, -math.inf, math.inf, True)
            else:
    ...
    plt.tight_layout()
    plt.savefig('C:/Users/MongkolChut/Downloads/New folder/GymRandom/Finished/PictureRandom/RewardperEpisodeforPlayersOfOfMiniMaxAlphaBeta.png')

if __name__ == "__main__":
    env = OthelloEnv()
    start_time = time.time()

    print("Random Agent vs Playing Minimax Agent with Alpha-Beta Pruning...")
    play_vs_minimax_alpha_beta(env, 5, depth=3)
