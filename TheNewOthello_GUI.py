...
def play_vs_random(env, episodes):
    ...
    white_label = tk.Label(root, text=f"White: {white_score}")
    white_label.pack(side="left")

    black_label = tk.Label(root, text=f"Black: {black_score}")
    black_label.pack(side="right")

    #show label for current player
    current_player_label = tk.Label(root, text=f"Current Player : {The_current_player}", anchor="center")
    ...
    for episode in range(1, episodes + 1):
        ...
        while not done:
            ...
            if current_player == BLACK:
                ...
                canvas.bind("<Button-1>", lambda event: play(min(event.x // 50, 63), min(event.y // 50, 63)))
